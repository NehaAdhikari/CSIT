...
void main_menu()
{
	...
	do
    {
        ...
        gotoxy(10,13); printf("4. Edit Transactions");
		gotoxy(10,15); printf("5. Delete Transactions");
		gotoxy(10,17); printf("6. Change Username/Password");
        gotoxy(2,21); printf("7 [Exit]");

        gotoxy(1,19); for(i=0;i<78;i++) printf("_"); //prints long line of "_" to separate menu and choice input
        gotoxy(23,21); printf("Press a choice between the range [1-7] ");
        ...
        switch(getch()-48) //getch gets the int value of "charater" 1 to 5, to convert it to respective integer we subtract it by 48
        {
		    ...
		    default://when entered characted is not between 1-6
		    {
		        title();
		        gotoxy(12,12); printf("Your input is out of range! Enter a choice between 1 to 6!");
...
void edit()
{
    ...
    title();
	        ...
	        fp=fopen("USER.DAT","r");
	        while(fscanf(fp,"%s %s\n",nam,pass)!=EOF) //opeans USER.DAT and reads the content
	        {
	            if(strcmp(user.name,nam)==0&&strcmp(user.password,pass)==0) c++; //if input username and password matches c is increased
	        }
	
	        fclose(fp);
	        title();
	        if (c==0) //input username and password didn't match so c is not increased hence c=0
	        {
	           ...
	           gotoxy(8,10); printf("Are you sure you want to CHANGE user name and/or password? <Y/N> : ");
	            ...
	            if(ch=='Y'||ch=='y')
	            {
	                do
	                {
	                    ...
	                    if (strcmp(n_pass,n_pass1)!=0) //comparing string of password and string of confirm password
	                    {
	                        title();
	                        gotoxy(17,13);
	                ...
	                tfp=fopen("TEMP.DAT","a"); //opeans a temporary file
	                fprintf(tfp,"%s %s\n",n_nam,n_pass); //prints the new username pad password in it
	                fclose(tfp);
	                fclose(fp);
	                remove("USER.DAT"); //delete the old USER.DAT file
	                rename("TEMP.DAT","USER.DAT"); //rename the TEMP.DAT into USER.DAT
	                title();
    ...
    int c,i,j=1,k=0;
 	title();
        ...
        switch(getch()-48)
        {
	    	...
	    	gotoxy(3,22);printf("Enter transaction no. to delete or press any other key to go back: ");
	    	scanf("%d",&c);
	    	fp=fopen("INCOME_T.DAT","r");
...
void tran_edit()
{
    ...
    int c,i,j=1,k=0;
	    ...
	    case 1:
			...
			fclose(fp);
			gotoxy(1,20); for(i=0;i<78;i++) printf("_");
	    	gotoxy(3,22);printf("Enter transaction no. to edit or press any other key to go back: ");
	    	scanf("%d",&c);
	    	fp=fopen("INCOME_T.DAT","r");
			tfp=fopen("TEMP.DAT","w");
	    	while(fread(&trans,sizeof(trans),1,fp)==1)
		        ...
		        {	title();
			        ...
			        switch(getch()-48)
			        {
						...
						case 3:
							...
							gotoxy(35,13);scanf("%f",&trans.amt);
            				fwrite(&trans,sizeof(trans),1,tfp);
            				title();
			    			gotoxy(20,12);printf("Transaction has been sucessfully Edited!");
			    			getch();
...
void tran_main_inc(int type)
{
	...
	while(fread(&trans,sizeof(trans),1,fp)==1)
	{
	    if(inputc<=trans.c) //to check the transaction counter and increase by 1
	    inputc++;
	}
	fclose(fp);
	trans.c=inputc;
    ...
    switch(getch()-48)
    {
    	case 1:
    	{
			...
			title();
		    currency(curr,trans.amt); //calls currency function which converts eg. "1234" to "Rs. 1,234"
