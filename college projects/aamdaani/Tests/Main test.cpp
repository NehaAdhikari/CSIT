...
//Log in Screen begins...
    ...
    do
    {
        ...
        else// when no records are found c=0
        {
...
void disp_main(int type)
{
	...
	switch(type)
	{
        ...
        gotoxy(53,6);puts("Time");
        gotoxy(65,6);puts("Amount (NRs.)");
        gotoxy(1,7); for(i=1;i<79;i++) printf("%c",196);
        
        fp=fopen("INCOME_T","r");
        while(fread(&trans,sizeof(trans),1,fp)==1)
        {
        	gotoxy(2,9+i);printf("%d",i);
        	gotoxy(8,9+i);printf("%s",trans.t_type);
        	strcpy(date,trans._date);
        	date_output(date);
        	gotoxy(36,9+i);printf("%s",date);
        	gotoxy(53,9+i);printf("%s",trans._ttime);
        	gotoxy(65,9+i);printf("%.2f",trans.amt);
        	i++;
		}
		fclose(fp);
		gotoxy(1,20); for(i=0;i<78;i++) printf("_");
    	gotoxy(23,22);printf("Press Backspace to go Back.");
    	c=getch();
    	if(c==8)
    	display();
    	else
...
void welcome()
{
    ...
    gotoxy(25,6); for(i=0;i<30;i++) printf("%c",196);
    gotoxy(35,7); printf("Project By:");
    gotoxy(25,8);for(i=0;i<30;i++) printf("%c",196);
    gotoxy(33,10); printf("Namkong Hang Kirat");
    gotoxy(33,12); printf("Neha Adhikari");
    gotoxy(33,14); printf("Kusal Kc");
    gotoxy(33,16); printf("Mala Thapa Magar");
    gotoxy(29,22);
